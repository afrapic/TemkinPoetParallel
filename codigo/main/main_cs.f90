...
Program sturmianTPCS
   !........ Parallel Version
    ...
    open(unit=1000,file='coeficientes/sol_N'//ord//'_lda'//lda//'_a'//al//'_out'//out//'.dat')
    do i=1,NN
 ...
 !  Calcula e imprime el flujo de PsiSc(r1,r2)
	...
	write(mth,'(a3)') 'sup'
        ...
        write(rh,'(i2)') int(rf)
	open(unit=1234,file='CSTP_N'//ord//'_lda'//lda//'_a'//al//'_out'//out//'_'//mth//'_r'//rh//'.dat')
	do i=0,ninterv
		if(i.eq.0)then
            	k1=0.05d0
        	else
	...
	write(1234,'(f20.10,1x,f20.10)') k1*k1/2d0,cs1(i)
	enddo
        endif
    enddo
    deallocate(cs1)
